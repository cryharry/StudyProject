...
public class BoardDAO {
	...
	public void insertBoard(BoardBean boardbean){
		...
		try {
			...
			//3 sql insert
			sql="insert into board(num,name,passwd,subject,content,file,date,readcount,ip,re_ref,re_lev,re_seq) values(?,?,?,?,?,?,?,?,?,?,?,?)";
			pstmt=conn.prepareStatement(sql);
			pstmt.setInt(1, num);
			...
			pstmt.setTimestamp(7, boardbean.getDate());
			pstmt.setInt(8, 0);
			pstmt.setString(9, boardbean.getIp());
			pstmt.setInt(10, num); //re_ref == 주글num
			pstmt.setInt(11, 0); //re_lev
			pstmt.setInt(12, 0);  //re_seq
			//4 실행
			pstmt.executeUpdate();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void reInsertBoard(BoardBean boardBean) {
	    ...
	    try {
            ...
            pstmt.setInt(8, 0);
            pstmt.setString(9, boardBean.getIp());
            pstmt.setInt(10, boardBean.getRe_ref()); //re_ref == 주글num
            pstmt.setInt(11, boardBean.getRe_lev()+1); //re_lev
            pstmt.setInt(12, boardBean.getRe_seq()+1);  //re_seq
